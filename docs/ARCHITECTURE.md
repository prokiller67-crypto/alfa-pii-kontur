...
Счётчика переключения фазы нет. При гонке Redis SET NX сохраняет одного победителя;
...
- В Redis/памяти находятся только ciphertext AES-256-GCM и ключи, полученные HMAC.
...
- API и модель работают локально. Runtime не вызывает внешнюю LLM и не передаёт текст NLP-провайдерам.
