...
files = [root / p for p in ("README.md", "pyproject.toml", "uv.lock", "Dockerfile", "compose.yaml", ".dockerignore", ".gitignore")]
for directory, extensions in {"src": {".py", ".html"}, "tests": {".py"}, "scripts": {".py"},
                              "benchmarks": {".py"}, "config": {".json"}, "docs": {".md", ".yaml"}}.items():
...
with ZipFile(out) as archive:
    ...
    if env.exists():
        for line in env.read_text().splitlines():
            ...
            if name.endswith("KEY") and len(value) > 20:
